...
**通しの遅延はほとんど変わらない。** 直結の実利は**帯域が倍**になることと、
**ばらつきが減る**こと（最大 132 → 102 ms）。

#### 75 ms の内訳を測った ★

同じ測定を PC-C 内（localhost）でも回して分解した。

| 成分 | 時間 | 詰められるか |
|---|---|---|
| 符号化＋復号（x264 ultrafast + avdec） | **1.5 ms** | もう限界 |
| **フレーム 1 枚ぶん**（30 fps） | **33 ms** | フレームレートを上げる以外に無い |
| **OME の固定オーバヘッド** | **約 21 ms** | OME の設定を変えるか、OME を通さない |
| 網＋TCP の戻り（直結） | 約 20 ms | 直結済みでこれ以上は減らない |

求め方: localhost だけで 30 fps=55 ms、60 fps=38 ms。差の 17 ms が
フレーム間隔の差（33.3→16.7 ms）と一致するので、
**OME = 固定 21 ms + フレーム 1 枚**と分かる。

確かめた（効かなかった）こと:

- 受信側の jitter buffer（`webrtcbin` の `latency`）を 0〜200 ms で振っても
  **変化なし**。ここは原因ではない
- キーフレーム間隔（`key-int-max` を 5〜90）でも**変化なし**。
  OME は GOP 単位でバッファしているわけではない
- 符号化と復号だけなら 1.5 ms しかかからない（OME を通さない経路で実測）

フレームレートを変えて検算した。**モデルとよく合う**:
...
**「1 フレームぶん」は帧率の物理的な下限ではなく、どこかの要素が
1 枚余計に抱えている。** 符号化・復号だけなら 30 fps でも 1.5 ms しか
かからないので、**抱えているのは x264 でも flvmux でもなく、OME か
WebRTC の受信側**（RTP の H.264 depayloader が「次のフレームの先頭パケット」
を見るまで現フレームを確定できない、という RTP 映像で典型的な 1 フレーム遅延
の可能性が高い）。

つまり「情報 1 つが伝わる時間」として意味のある数字は
**fps に依らない固定分の約 41 ms**（OME 21 + 網 20）で、
残りの 33 ms は管の性質。

**結論: 詰める余地は小さい。** 固定分 41 ms は既に十分小さく、
VLM の推論時間（おそらく数百 ms）がこの上に乗るので、
**ここを削っても全体はほとんど変わらない。**
本気で 1 フレームぶんを削るなら OME を置き換えるか webrtcbin の
depay の挙動に踏み込むことになり、割に合わない。**今はやらない。**
