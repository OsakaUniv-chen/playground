...
| 項目 | 状態 |
|---|---|
| **直結にならない** | 常に DERP 中継（`relay "tok"`）。両側とも `MappingVariesByDestIP: false`・UDP 可・UPnP 有りで条件は良いのに穴が開かない。理研側の機器が入力 UDP を塞いでいる疑い。**帯域は足りているので当面は困らない**が、遅延と Tailscale の中継への依存は残る |
| 遅延の実測 | DERP 経由の RTT は 26–125 ms とばらつく。首の動きが発話に対して遅れないかは通しで見る必要がある |
