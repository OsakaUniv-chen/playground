...
**結論: 詰める余地は小さい。** 33 ms は 30 fps である以上避けられず、
残りは OME の 21 ms と網の 20 ms。しかも VLM の推論時間（おそらく
数百 ms）がこの上に乗るので、**ここを削っても全体はほとんど変わらない。**
本気で削るなら「PC-D を OME 経由でなく PC-B から直接受ける」ことになるが、
PC-B を tailnet に入れる必要が生じるうえ、操作者が見る経路と分岐して
構成が複雑になる。**今はやらない。**
