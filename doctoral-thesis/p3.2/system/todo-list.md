...
| 項目 | 状態 |
|---|---|
| 直結にならない | 常に DERP 中継（`relay "tok"`）。両側とも `MappingVariesByDestIP: false`・UDP 可・UPnP 有りで条件は良いのに穴が開かない。理研側が入力 UDP を塞いでいる疑い。**帯域 22 Mbps・遅延 78 ms がどちらも足りているので、実害は無い。** PC-C は全球 IPv6 を持つので、理研側で IPv6 が使えれば NAT 自体が無くなって直結する見込みはある |
