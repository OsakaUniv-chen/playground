...
| パラメータ | 既定値 | 確認方法 |
|---|---|---|
| `max_pitch` / `max_yaw` / `max_arm` | ±30 / ±60 / ±90 度 | 機体やケーブルに当たらない範囲を実測。**カメラと 16ch アレイは機体に固定で、頭には載らない**（カメラは 200 度の広角なので首を振らなくても全体が写る。首が動くのは人に見せるため） |
| `speed` / `acceleration` / `torque` | 20.0 / 200.0 / 0.2 | 頭部の実際の重さで詰める |
| `HEAD_MOUNT_YAW_DEG` / `_PITCH_DEG`（`pc-d-server/config.env`） | 0 / 0 | **アレイの正面と頭の 0 度のずれ。** 頭の 0 度は起動時の姿勢なので、ふつう一致しない。正面に立って手を叩き、`soundmap_geometry` が出す角度と、実際に正面を向く指令角の差を測る。**固定のオフセットなので溜まる誤差にはならない** |
