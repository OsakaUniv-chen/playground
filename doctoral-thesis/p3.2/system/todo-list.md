...
| 項目 | 状態 |
|---|---|
| 直結にならない | **原因は PC-C の VPN。下記。** 帯域 22 Mbps・遅延 78 ms がどちらも足りているので実害は無いが、直せば遅延はさらに縮む |
...
| 手 | 中身 |
|---|---|
| **VPN を切る** | **Tailscale が通った今、踏み台 SSH は不要**（`ssh chen@100.104.252.121` で直接入れる）。VPN に他の用途が無ければこれが一番きれい |
| 例外経路を足す | `sudo ip route add 134.160.214.180/32 via 192.168.1.1 dev wlo1`。VPN 再接続で消えるので、恒久化するなら VPN 側の設定で除外する |

確認は `tailscale ping 100.104.252.121` が `via DERP(tok)` から
`via 134.160.214.180:...` に変わるかで見る。
