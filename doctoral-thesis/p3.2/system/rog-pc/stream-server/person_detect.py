...
**不发时间戳**，robot-pc 按自己收到的时刻处理，两机不需要 NTP 同步（架构 §6.4）。
...
**★ 能直接跑原图，靠的是「人是竖直的」。** 前向安装时人的朝向和方位角无关，
所以 COCO 预训练的 person 类直接能用。**换成朝上的全向鱼眼（会议相机那种）
这条就不成立了** —— 人的朝向会随方位角旋转，必须先展开成全景条带再检测，
否则边缘的人全漏。见 README「换相机的话」。
...
**顺便推一条监视流回 OME**（stream key `human_detect`，5 fps）：鱼眼 ＋ 检测框
＋ 右上角的录制标志。**只是给人看的**，不进 bag、不参与任何判断。出口在
srt_out.py，编码器堵住会丢帧而不会拖慢这里的循环。
...
# 和输入尺寸最吃紧的地方。
#
# x@1088 实测中位 97.8 ms，5 Hz 的预算是 200 ms —— 只用掉一半。
# 各组合的耗时见 README「检测」。
MODEL = "yolo11x.pt"
...
# 漏干（回到 0）才落下。**★ 不要改成「连续 N 帧没人」** —— 「连续」对偶发误检
# 没有免疫力，每隔几秒一帧误检就能让计数永远凑不齐，置位之后再也落不下来。
#
# 关键性质是能算出来的：设某个东西在 p 比例的帧里被检到，
