...
#
# 上面四条走 SRT 进 OME，用 streamid 区分。**speaker 是反方向的那一条**：
# 出 OME 一律 WebRTC，所以它不是 gst-launch 而是 speaker.py（见那边的注释）。
#
# **管线都在 Python 里（cam.py / soundmap.py / speaker.py），这个脚本只管
# 解析设备、起进程、看着它们。** 记录要的采集时刻只有持有管线的进程算得出来，
# 命令行发不了 ROS 消息 —— 所以三路相机也从 gst-launch 搬进了 cam.py。
#
# 每条流都由一个监视循环看着，退出就重起。**设备解析在循环里面** ——
# 每次重起都重新按名字找 /dev/videoN、重新按名字找声卡，所以拔插之后设备号
# 变了也能自己接回来（详见 __run 那一段的注释）。
#
# 光看「进程退没退」发现不了 USB 抽风、驱动卡死这种**管线还活着但不出数据**的
# 故障，所以**每个进程自己盯自己**：cam.py 数 srtsink 那端出去的包、
# soundmap.py 数进来的音频，多久没动就写个 .stalled 记号再退出（见 __supervise）。
#
# 用法:
...
# ---------------------------------------------------------------------
# 三路相机。**管线在 cam.py 里**，这里只做两件事：按名字解析设备
# （每次重起都重解析，所以拔插换了 /dev/videoN 也能接回来），
# 以及把记录的开关传下去。
# ---------------------------------------------------------------------
...
#
# 顺带分出这一轮是**自己退的**还是**卡死被自己打死的**：两者都是重起，但原因
# 完全不同 —— 前者多半是 OME / 网络，后者是 USB 或驱动。状态面板分两列显示。
#
# **卡死是各个进程自己发现的**（cam.py 数 srtsink 那端的包、soundmap.py 数
# 输入），发现了就往 <名字>.stalled 写个记号再退出。这里只负责记账 ——
# 判断放在持有管线的进程里，比在外面猜日志准得多。
__supervise() {
