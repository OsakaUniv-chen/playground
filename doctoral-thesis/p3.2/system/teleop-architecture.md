...
この範囲は実装済み。**§4.3 の「誰に向くか」の判断（`decide()`）は未実装**で、
音響マップ上の位置から yaw/pitch への対応付けは導出済み（`pc-d-server/soundmap_geometry.py`、生成器の写像と突き合わせて中央値 0.13 度）。
...
### 4.2 音声の文字起こし（VLM の入力を作る）

4 入力のうち**音声 2 本は、そのままでは VLM に渡せない**ので、PC-D の中で
文字にしてから渡す。faster-whisper（既定 `medium`）を GPU で回す。

| | |
|---|---|
| 切り出し | **無音で区切る。固定長では切らない** ── 語の途中で切れて境目に欠落と重複が出るため。喋りの判定は暗騒音への相対値（固定閾値だと会場ごとに破綻する） |
| 文脈 | VLM に渡すのは**直近 15 秒**（`ASR_CONTEXT_SEC`）。判断は「頭を誰に向けるか」なので、直近 2〜4 ターンで足りる。長くすると終わった話題の話者に引きずられる |
| 音源 | `mic`（現場）と `operator`（操作者）を**分けたまま持つ**。「操作者が今なにか聞いた」のか「来場者が喋った」のかで向く先が変わる |

**この 2 つの窓は別物**（前者は音の切れ目、後者は判断に要る文脈の長さ）。
VLM を繋いだあとに調整するのは後者だけ。
...
PC-B のモータまで届ける所は通してある。

**カメラと 16ch アレイは機体に固定で、頭とは一緒に動かない。** カメラは
200 度の広角なので首を振らなくても場面全体が写る ── 首が動くのは「その人の
ほうを向いた」と**人に見せるため**。したがって音響マップの中心は常に機体の
正面で、そこから読める角度はそのまま絶対角として使える（画素との対応付けは
`pc-d-server/soundmap_geometry.py` で導出・検算済み）。

実機で測るのは**アレイの正面と頭の 0 度のずれ**だけ。頭の 0 度は起動時の
姿勢なので両者はふつう一致しないが、**固定のオフセットなので溜まる誤差には
ならない。**
