...
**音響マップの角度は絶対角。足し込みは無い。** 16ch アレイとカメラは
**機体に固定**されていて頭とは一緒に動かない ── カメラは 200 度の広角なので
首を振らなくても場面全体が写る。首が動くのは「その人のほうを向いた」と
**人に見せるため**であって、センサを向けるためではない。だからマップの中心は
常に機体の正面で、頭がどこを向いていても見え方は変わらない。

校正は 1 つだけ、**アレイの正面と頭の 0 度のずれ**（`HEAD_MOUNT_YAW_DEG` /
`HEAD_MOUNT_PITCH_DEG`）。頭の 0 度は起動時の姿勢なので両者はふつう一致
しない。`HeadController.look_at()` がこれを足す。**固定のオフセットなので、
溜まっていく類の誤差にはならない。**
...
`decide()` は未実装。**戻り値は絶対角 `(yaw, pitch)`** と決めてある。
