...
**16ch アレイとカメラは機体に固定されていて、頭と一緒には動かない。**
カメラは 200 度の広角なので、首を振らなくても場面全体が写る ── 首が動くのは
「その人のほうを向いた」と**人に見せるため**であって、センサを向けるためでは
ない。

したがってマップの中心は**常に機体の正面**で、ここが返す角度はそのまま
絶対角として使える。**前の指令に足し込まないこと** ── 動くのは頭だけで、
マップの見え方は頭がどこを向いていても変わらない。

残る校正は 1 つだけで、**アレイの正面と頭の 0 度がどれだけずれているか**。
頭の 0 度は起動時の姿勢（`preset_position(0)`）なので、機体に組んでから
実測して `HEAD_MOUNT_YAW_DEG` / `HEAD_MOUNT_PITCH_DEG` に入れる。
**固定のオフセットなので、溜まっていく類の誤差にはならない。**
"""
