#!/usr/bin/env python3
"""Render final 1-bit RGB + sound-map overlay videos for Demonstration_Data.

Final decision:
  band-pass: 2000-8000 Hz
  filter order: 4
  mic pairs: all 120 pairs
  delay: exact integer delay
  display: max(raw - p99(raw), 0) + per-frame minmax
"""
...
def p99_minmax(raw_sm: np.ndarray) -> np.ndarray:
...
def overlay_frame(frame_bgr: np.ndarray, raw_sm: np.ndarray, t_s: float) -> np.ndarray:
    cam = cv2.resize(frame_bgr, (OUTPUT_SIZE, OUTPUT_SIZE), interpolation=cv2.INTER_AREA)
    sm_color = sm_to_color(p99_minmax(raw_sm), OUTPUT_SIZE)
    blend = cv2.addWeighted(sm_color, SM_ALPHA, cam, CAM_ALPHA, 0)
    cv2.rectangle(blend, (0, 0), (OUTPUT_SIZE, 64), (0, 0, 0), -1)
    cv2.putText(blend, "1-bit final: 2000-8000 Hz, all pairs, exact delay", (16, 29),
...
def render_bag(bag_dir: Path, out_dir: Path, limit_frames: int | None = None) -> Path:
    ...
    out_path = out_dir / f"{bag_dir.name}_1bit_final.mp4"
    tmp_video = out_dir / f"_{bag_dir.name}_1bit_final_video.mp4"
    tmp_audio = out_dir / f"_{bag_dir.name}_1bit_final_audio.wav"
    ...
    try:
        for frame_no, ci in enumerate(frame_indices, start=1):
            ...
            if ja <= 0:
                ...
                raw_sm = api.generate_raw_score(a_d[max(0, ja - AUDIO_WIN):ja])
            writer.write(overlay_frame(frame, raw_sm, (t - t0) / 1e9))
            written += 1
    ...
    wavfile.write(tmp_audio, FS, np.clip(mono, -32768, 32767).astype(np.int16))
