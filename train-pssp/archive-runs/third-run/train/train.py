"""Training loop for the PSSP SimVP model -- third-run: data-scale test.

third-run's single variable vs. first-run is DATA (see dataset.py's
docstring for the full reasoning): all 78 WordWolfExp bags plus chat's first
two debate sessions for training, chat's third session held out as the only
test bag. Everything else is deliberately unchanged from first-run so any
metric difference is attributable to the bigger/more-diverse training set,
not a confounded architecture/loss/hyperparameter change:
  - Single GPU, no DDP.
  - No data augmentation.
  - Plain MSE loss on the exp(x-max)-normalized target (second-run's loss
    ablation found the loss function wasn't the bottleneck -- see CONTEXT.md
    -- so this run doesn't re-open that variable).
  - lr=1e-3 (AdamW), same as first-run.
