"""Training loop for the PSSP SimVP model on train-pssp/train-data/.

Corresponds to old-train-scripts/train_all_load.py, rewritten for this
project's settled decisions (see train-pssp/CONTEXT.md):
  - Single GPU, no DDP -- current data volume (a few thousand windows) doesn't
    need it; DDP can come back if/when this moves to the full 78-bag dataset.
  - No data augmentation yet -- get a working baseline first, decide whether
    flip/random-crop-style augmentation is worth adding once there's something
    to compare against.
  - Plain MSE loss on the exp(x-max)-normalized target -- now scale-consistent
    with the model's sigmoid output (see CONTEXT.md's normalization write-up),
    unlike the old pipeline's suspected (and never confirmed) mismatch.
  - lr=1e-3 (AdamW), not the old 1e-6 -- that value looked suspiciously low
    and is a plausible second reason (besides normalization) the old model
    undertrained; configurable via --lr.
