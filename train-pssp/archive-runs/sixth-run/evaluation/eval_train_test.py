...
first-run/evaluation/eval_train_test.py for the full rationale. Each of
sixth-run's 6 checkpoints was trained on either the sparse (2Hz) or dense
(~30Hz-native) chat train set -- RUN_DENSITY below records which, so the
train-set eval uses the SAME density each checkpoint actually trained on
(the shared test set is always sparse, per dataset.py's own convention).
"""
...
from dataset import SparseChatWindowDataset, DenseChatWindowDataset
...
# dense train set (~87k windows) makes a full pass slow across 4 checkpoints;
# stride-subsample down to ~this many for a fast, still-representative estimate.
MAX_TRAIN_WINDOWS = 30000

CLIP_LEN, PRED_LEN, SM_RATIO = 10, 4, 0.5

RUN_DENSITY = {
    "ablation1_sparse_oldlr": "sparse",
    "ablation1_dense_oldlr": "dense",
    "ablation2_dense_newlr": "dense",
    "ablation3_dense_newlr_aug": "dense",
    "extra_sparse_newlr_aug": "sparse",
    "extra_sparse_newlr_noiseratio": "sparse",
}
...
def main():
    ...
    lines = [f"## {time.strftime('%Y-%m-%d %H:%M')} train/test 分列复核（新指标口径：逐步不合并，重点t+2）", "",
             "6 个 checkpoint 各自的训练集密度不同（sparse=2Hz / dense=~30Hz原生起点），"
             "见脚本 RUN_DENSITY，train 指标用各自实际训练时用的那份数据算。测试集统一是 "
             "sparse 版 chat 3 个bag各自最后10%。exp4 不在训练集上评估（训练数据边界不同）。", ""]

    sparse_train_ds = SparseChatWindowDataset("train", clip_len=CLIP_LEN, pred_len=PRED_LEN, sm_ratio=SM_RATIO)
    dense_train_ds = DenseChatWindowDataset(clip_len=CLIP_LEN, pred_len=PRED_LEN, sm_ratio=SM_RATIO)
    dense_n_total = len(dense_train_ds)
    dense_note = ""
    if dense_n_total > MAX_TRAIN_WINDOWS:
        stride = -(-dense_n_total // MAX_TRAIN_WINDOWS)  # ceil div
        dense_train_ds = Subset(dense_train_ds, range(0, dense_n_total, stride))
        dense_note = f"（等距抽样自 {dense_n_total} 个窗口）"
    train_loaders = {
        "sparse": DataLoader(sparse_train_ds, batch_size=64, shuffle=False, drop_last=True),
        "dense": DataLoader(dense_train_ds, batch_size=64, shuffle=False, drop_last=True),
    }
    train_notes = {"sparse": "", "dense": dense_note}
    print(f"sparse train: {len(sparse_train_ds)} windows, dense train: {len(dense_train_ds)}/{dense_n_total} "
          f"windows used, test: {len(test_ds)} windows")

    baseline_fn = lambda x: baseline_repeat_last(x, PRED_LEN, SM_RATIO)
    for run_name, density in RUN_DENSITY.items():
        model = load_checkpoint(RUNS_DIR / run_name / "best_model.pt", shape_in, PRED_LEN, "gsta", device)
        models = {f"sixth-run ({run_name})": lambda x, m=model: m(x), "baseline (repeat-last)": baseline_fn}

        train_pd, train_psr, n_train = compute_metrics(models, train_loaders[density], device)
        test_models = {"old (exp4)": lambda x: old_model(x), **models}
        test_pd, test_psr, n_test = compute_metrics(test_models, test_loader, device)
        print_report(f"TRAIN[{density}] {run_name}", train_pd, train_psr, n_train)
        print_report(f"TEST {run_name}", test_pd, test_psr, n_test)

        lines.append(f"### {run_name} (train density: {density})")
        lines.append("")
        lines += format_split_tables(f"TRAIN[{density}]{train_notes[density]}", train_pd, train_psr, n_train)
