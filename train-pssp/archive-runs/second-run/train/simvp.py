...
class Decoder(nn.Module):
    ...
    output_activation (second-run addition, for the loss-function ablation --
    see CONTEXT.md): 'sigmoid' (default, original behavior) restricts each
    pixel independently to [0,1], used for MSE/BCE loss. 'none' returns raw
    logits unchanged, used for the KL/spatial-softmax loss condition, where
    the softmax is applied over the whole map (not per-pixel) in the loss
    function itself, not here."""
    def __init__(self, C_hid, C_out, N_S, spatio_kernel, act_inplace=True, output_activation='sigmoid'):
    ...
    def forward(self, hid, enc1=None):
        ...
        if self.output_activation == 'sigmoid':
            Y = torch.sigmoid(Y)  # Apply sigmoid to restrict output between 0 and 1
        return Y
...
class SimVP(nn.Module):
    def __init__(self, in_shape, pred_len, hid_S=64, hid_T=256, N_S=4, N_T=4, model_type='gsta',
                 mlp_ratio=8., drop=0.2, drop_path=0.2, spatio_kernel_enc=3,
                 spatio_kernel_dec=3, act_inplace=True, output_activation='sigmoid', **kwargs):
        super(SimVP, self).__init__()
        # hid_S=64, hid_T=256, N_S=4, N_T=8, model_type='gsta', mlp_ratio=8., drop=0.2, drop_path=0.2, spatio_kernel_enc=3, spatio_kernel_dec=3
        ...
        self.dec = Decoder(hid_S, C, N_S, spatio_kernel_dec, act_inplace=act_inplace, output_activation=output_activation)
