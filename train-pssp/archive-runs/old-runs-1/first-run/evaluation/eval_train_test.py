...
per t+1..t+4 SEPARATELY (t+2 is the primary step, others are auxiliary context,
never collapse to one aggregate number as the decision criterion). See
CONTEXT.md "关键诊断指标" -- position correlation is retired, this replaces it.

exp4 (old model) is NOT evaluated on this run's train split -- exp4 was trained
on a different, unknown dataset, so "train metric" has no overfitting-diagnosis
meaning for it. It's still reported on test (existing convention). The naive
continuity baseline is always reported on both splits per owner's hard rule.

Appends a dated section to ../RESULTS.md, same format family as compare_old_new.py.
"""
...
def main():
    ...
    train_models = {
        "new (first-run baseline)": lambda x: new_model(x),
    ...
    lines = [f"## {time.strftime('%Y-%m-%d %H:%M')} train/test 分列复核（新指标口径：逐步不合并，重点t+2）", "",
             "训练集：`TRAIN_BAGS`（16 bag）。测试集：`TEST_BAGS`（8 bag，G2+G6）。exp4 不在训练集上评估"
             "（它的训练数据和这里的 TRAIN_BAGS 无关，训练集指标对它没有过拟合诊断意义，仅在测试集上作为参照）。",
             ""]
