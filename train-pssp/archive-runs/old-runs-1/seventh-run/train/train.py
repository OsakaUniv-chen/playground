"""Training loop for the PSSP SimVP model -- seventh-run: scaling sixth-run's
chat-only ablation result up to the full dataset.

sixth-run (chat only, 3 bags, see CONTEXT.md) found that sparse (2Hz, no
dense re-extraction) + lr=1e-3 + data augmentation (noise+sm_ratio jitter,
see augment.py -- a new scheme, not exp4's flip+crop, chosen because it
matched flip+crop almost exactly while being cheaper and avoiding any
spatial-geometry risk) matches/slightly beats exp4 on peak_dist/PSR/position
correlation, and that the dense sliding-window density from ablation #1
isn't actually necessary once lr+augmentation are both right. seventh-run
applies exactly that recipe (sparse + lr=1e-3 + augment=noise_ratio) to
fifth-run's full 278-bag training pool (dataset.py, TRAIN_BAGS/TEST_BAGS
unchanged from fifth-run) to see whether the chat-only finding generalizes:
  - Single GPU, no DDP.
  - lr=1e-3 (AdamW).
  - noise_ratio augmentation on every training batch (never at eval time).
...
from dataset import make_datasets, TRAIN_BAGS, WORDWOLF_AND_CHAT_TRAIN_BAGS
...
def main():
    ...
    ap.add_argument("--augment", choices=["none", "flipcrop", "noise_ratio"], default="noise_ratio",
                     help="'flipcrop' replicates exp4's real training-time augmentation; "
                          "'noise_ratio' (default) is sixth-run's winning scheme -- see augment.py")
    ap.add_argument("--train-scope", choices=["full", "wordwolf_chat"], default="full",
                     help="'full' = all 278 bags; 'wordwolf_chat' = WordWolfExp(78)+chat(2) only, "
                          "a middle-scale subset to isolate whether dataset SIZE (not the newly "
                          "added diversity) explains why the full run's early-stop was so early "
                          "-- see dataset.py's WORDWOLF_AND_CHAT_TRAIN_BAGS docstring")
    ...
    print(f"run: {run_dir}  augment={args.augment}  lr={args.lr}  train_scope={args.train_scope}")

    train_bags = TRAIN_BAGS if args.train_scope == "full" else WORDWOLF_AND_CHAT_TRAIN_BAGS
    train_ds, test_ds = make_datasets(
        DATA_DIR, train_bags=train_bags, clip_len=args.clip_len, pred_len=args.pred_len, sm_ratio=args.sm_ratio,
    )
