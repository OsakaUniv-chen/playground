"""Sound-map generator: PyTorch FFT-power beamforming, no acoular dependency.

Validated in Documents/Playground/generator-compare/ (as the "new"/torch
generator there) against the old acoular-based one across 65 bags, ~50k
ticks: 4-label agreement 1.000, raw map Pearson r~=0.99999, 7.7x faster. See
train-pssp/CONTEXT.md for the full provenance. Renamed here (no "new" in the
name -- there's only one generator in this project) and the UMA-16 mic
geometry is now a constant in this file instead of a separate XML asset.
"""
import warnings
...
# UMA-16 (miniDSP) mic geometry, (x, y, z) meters. Was minidsp_uma-16.xml.
MIC_POSITIONS = (
    (0.021, -0.063, 0.0),
    (0.063, -0.063, 0.0),
    (0.021, -0.021, 0.0),
...
class SoundMapGenerator:
    def __init__(
        ...
        self.mpos = self._load_mic_positions()
    ...
    def _load_mic_positions(self):
        mpos = np.array(MIC_POSITIONS, dtype=np.float64).T  # (3, channels)
        if mpos.shape != (3, self.channels):
            raise ValueError(f"Expected microphone geometry with shape (3, {self.channels}), got {mpos.shape}")
        return mpos
