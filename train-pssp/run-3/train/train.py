...
run-3 (2026-07-21) adds --expo-holdout/--indy-holdout (see dataset.py). Like
--atr1f-holdout before them, these are intentionally NOT added to DOMAINS/
eval_loaders below -- early stopping keeps watching only the three
already-validated domains, never the domain under test itself; expo/indy's
held-out performance is scored post-hoc by report.py --extra-eval-domain,
same pattern as atr1f.
"""
...
def main():
    ...
    ap.add_argument("--expo-holdout", action="store_true", dest="expo_holdout",
                     help="add EXPO_TRAIN_BAGS (155 bags) into combined_train, keeping expo_reception_2025's"
                          " 2025-09-29 (12 bags) wholly unseen -- run-3, see dataset.py")
    ap.add_argument("--indy-holdout", action="store_true", dest="indy_holdout",
                     help="add INDY_TRAIN_BAGS (17 bags) into combined_train, keeping indy_teleoperation's"
                          " 2026-04-09 (6 bags) wholly unseen -- run-3, see dataset.py")
    ...
    ds = make_datasets(clip_len=args.clip_len, pred_len=args.pred_len, sm_ratio=args.sm_ratio,
                        ...
                        atr1f_holdout=args.atr1f_holdout, expo_holdout=args.expo_holdout,
                        indy_holdout=args.indy_holdout)
