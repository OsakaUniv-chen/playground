...
def augment_batch_both(x: torch.Tensor, y: torch.Tensor, base_ratio: float = 0.5,
                        ratio_range: tuple[float, float] = (0.3, 0.7),
                        noise_std: float = 0.03) -> tuple[torch.Tensor, torch.Tensor]:
    """Stacks flip_and_crop (spatial, both x and y) with noise_ratio jitter
    (signal, x only) -- the two schemes touch orthogonal aspects of the
    batch so composing them is a straightforward pipeline: geometry first,
    then signal/channel perturbation on the (already flipped/cropped) input."""
    x, y = augment_batch(x, y)
    return augment_batch_noise_ratio(x, y, base_ratio=base_ratio, ratio_range=ratio_range, noise_std=noise_std)
