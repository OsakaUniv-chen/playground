...
class LazyBfResult:
    ...
    def shape(self):
        # Assuming self.bf._ac is a numpy array or has a shape attribute
        return self.bf._ac.shape
...
class BeamformerBase(HasPrivateTraits):
    ...
    def synthetic(self, f, num=0):
        ...
        if num == 0:
            ...
            # Result Shape: (freq bins num, map_x * map_y)
            #print(f'f diff:{freq[1]-freq[0]}  f_min:{f1}  f_max:{f2}, result shape:{res.shape()} ind1={ind1} ind2={ind2}')
    ...
    def _calc(self, ind):
        ...
        for i in ind:
            if self.r_diag:
                ...
                eva = clip(array(self.freq_data.eva[i], dtype='float64'), 1e-10, None) ** (1.0 / self.gamma)
...
class BeamformerCapon(BeamformerBase):
    ...
    def _calc(self, ind):
        ...
        for i in ind:
            reg_param = 1e-5
            csm = array(self.freq_data.csm[i], dtype='complex128')
            csm += reg_param * eye(csm.shape[0])
            csm = array(linalg.inv(csm), order='C')
