...
class SphericalGrid(Grid):
    """Provides a spherical grid with fixed z-coordinate for the beamforming results.
    
    The grid is defined by the distance along the z-axis, and the azimuth and
    elevation angle ranges, with a given increment.
    """
    
    #: The fixed z-coordinate that defines the grid, defaults to 1.
    distance = Float(1.0, desc='distance along z-axis')

    #: The azimuth angle range that defines the grid, defaults to 40.
    azimuth_range = Float(40.0, desc='azimuth angle range')

    #: The elevation angle range that defines the grid, defaults to 30.
    elevation_range = Float(30.0, desc='elevation angle range')

    #: The angle increment for the grid, defaults to 1.0.
    angle_increment = Float(1.0, desc='angle increment in degrees')

    # internal identifier
    digest = Property(
        depends_on=['distance', 'azimuth_range', 'elevation_range', 'angle_increment'],
    )

    @property_depends_on('distance, azimuth_range, elevation_range, angle_increment')
    def _get_size(self):
        az_steps = int(2 * self.azimuth_range / self.angle_increment) + 1
        el_steps = int(2 * self.elevation_range / self.angle_increment) + 1
        return az_steps * el_steps
    ...
    @property_depends_on('distance, azimuth_range, elevation_range, angle_increment')
    def _get_gpos(self):
        azimuths = np.arange(-self.azimuth_range, self.azimuth_range+self.angle_increment, self.angle_increment)
        elevations = np.arange(-self.elevation_range, self.elevation_range+self.angle_increment, self.angle_increment)
        ...
        for az in azimuths:
            for el in elevations:
                ...
                x = self.distance * np.tan(az_rad)
                y = self.distance * np.tan(el_rad)
                z = self.distance
                grid_points.append([x, y, z])
                
        return np.array(grid_points).T

    def index(self, az, el):
        pass

    def indices(self, *angles):
        pass


class FisheyeGrid(Grid):
    distance = Float(1.0)
    azimuth_range = Float(180/2)
    elevation_range = Float(101.25/2)
    pixel_x_range = Int(1890)
    pixel_y_range = Int(1080)
    pixel_increment = Int(45)
    # 1890， 30 -》 63
    # 1890， 45 -》 42
    
    digest = Property(
        depends_on=['distance', 'azimuth_range', 'elevation_range', 'pixel_x_range', 'pixel_y_range', 'pixel_increment'],
    )

    @property_depends_on('distance, azimuth_range, elevation_range, pixel_x_range, pixel_y_range, pixel_increment')
    def _get_size(self):
        x_steps = int(self.pixel_x_range / self.pixel_increment) + 1
        y_steps = int(self.pixel_y_range / self.pixel_increment) + 1
    ...
    @property_depends_on('distance, azimuth_range, elevation_range, pixel_x_range, pixel_y_range, pixel_increment')
    def _get_gpos(self):
        edge_x = int((1920 - self.pixel_x_range) / 2)
        xs = np.arange(edge_x, 1920-edge_x+1, self.pixel_increment)
        ys = np.arange(0, 1080+1, self.pixel_increment)

        grid_points = []
        for x in xs:
            for y in ys:
                az = (x - 960) / 960 * self.azimuth_range
                el = (y - 540) / 540 * self.elevation_range
                az_rad = np.deg2rad(az)
                el_rad = np.deg2rad(el)
                X = self.distance * np.tan(az_rad)
                Y = self.distance * np.tan(el_rad)
...
class XactiGrid(Grid):
    distance = Float(1.0)
    pixel_x_range = Int(1080)
    pixel_y_range = Int(1080)
    pixel_increment = Int(30)
    
    digest = Property(
        depends_on=['distance', 'pixel_x_range', 'pixel_y_range', 'pixel_increment'],
    )
    ...
    @property_depends_on('distance, pixel_x_range, pixel_y_range, pixel_increment')
    def _get_gpos(self):
        x_points = np.arange(420, 1501, self.pixel_increment)
        y_points = np.arange(0, 1081, self.pixel_increment)
        xy_points = np.array(np.meshgrid(x_points, y_points)).T.reshape(-1, 2).astype(np.float32)

        grid_points = []
        K = np.array([[360.01808365, 0., 956.08854188], [0. ,359.52442866, 535.9666882], [0., 0., 1.]])
        D = np.array([[-0.02388909], [-0.01300854], [0.01702502], [-0.00771782]])
        for point in xy_points:
            x, y = int(point[0]), int(point[1])
            distorted_points = np.array([[[point[0], point[1]]]], dtype=np.float32)
            undistorted_points = cv2.fisheye.undistortPoints(distorted_points, K, D, R=np.eye(3))
            undistorted_points_homogeneous = cv2.convertPointsToHomogeneous(undistorted_points)
            real_world_point = undistorted_points_homogeneous * self.distance

            X = real_world_point[0][0][0]
            Y = real_world_point[0][0][1]
            Z = self.distance
            grid_points.append([X, Y, Z])
