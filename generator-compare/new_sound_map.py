...
class NewSoundMapGenerator:
    def __init__(
        ...
        self.mpos = self._load_mic_positions(xml_path)
    ...
    def _select_device(self, requested):
        if requested == "cuda" and not torch.cuda.is_available():
            warnings.warn("CUDA requested for NewSoundMapGenerator, but it is not available. Falling back to CPU.", RuntimeWarning, stacklevel=2)
    ...
    def _load_mic_positions(self, xml_path):
        doc = xml.dom.minidom.parse(str(Path(xml_path)))
        xyz = []
        for el in doc.getElementsByTagName("pos"):
            xyz.append([float(el.getAttribute(a)) for a in "xyz"])
        mpos = np.array(xyz, dtype=np.float64).T
